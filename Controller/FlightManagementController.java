...
public class FlightManagementController<T, E> implements ControllerInterface<Flight, FlightManagementSystem> {
	...
	public TableModel refreshTickets(Flight flight) {
		// TODO Auto-generated method stub
		String[] columnNames = { "Modules.Ticket Type", "Seat Number", "Modules.Passenger", "Modules.Ticket Price","Modules.Passenger Id"};
		DefaultTableModel model = new DefaultTableModel(columnNames, 0);
	...
	public void buyTicket(Integer flightRow, Integer ticketRow, int passengerId, float ticketPrice) {
		...
		if (passenger == null) {
			...
			JOptionPane.showMessageDialog(null, "Modules.Passenger id does not exist", "Error", JOptionPane.ERROR_MESSAGE);

		} else {
			if (ticket instanceof VIPTicket && !(passenger instanceof BusinessPassenger)) {
				JOptionPane.showMessageDialog(null, "VIP Modules.Ticket can be sold to Business Modules.Passenger", "Error",
